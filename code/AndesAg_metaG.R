# Andes Ag soil column study on MP1 and Basalt addition
# Taxonomic Analysis of 72 metagenomes using mTAGs and Sylph
# By Cliff Bueno de Mesquita, Fierer Lab, 2025

# Note: n = 70 analyzed since 2 had low sequencing depth
# Note: 4 lanes of sequencing data were concatenated
# Note: Functional analysis not included here as it is not in the first paper
# Note: 3 main manuscript figures are produced here (Figures 5,6,7)
...
#write.table(d1, "data/metadata.txt", sep = "\t", row.names = F)
# Note: metadata.txt file has some formatting issues!
d1$sampleID
table(d1$Group)
table(d1$Replicate)
table(d1$Depth)
...
# Only need to run the setup once, then can restart with .rds files
...
pdf("InitialFigs/SoilPCA.pdf", width = 7, height = 5)
...
plot.with.inset
pdf("InitialFigs/FigureS6.pdf", width = 7, height = 5)
plot.with.inset
dev.off()
png("InitialFigs/FigureS6.png", width = 7, height = 5, units = "in", res = 300)
plot.with.inset
dev.off()
...
plot.with.inset.10
pdf("InitialFigs/Beta_10.pdf", width = 7, height = 5)
plot.with.inset.10
dev.off()
pdf("InitialFigs/Beta_10_wVec.pdf", width = 7, height = 5)
plot.with.inset.10
dev.off()
png("InitialFigs/Beta_10_wVec.png", width = 7, height = 5, units = "in", res = 300)
plot.with.inset.10
dev.off()
...
pdf("InitialFigs/Phylum_Barplot_10.pdf", width = 8, height = 6)
phyplot
dev.off()
...
phyplot2 <- ggplot(bars_phyla, aes(group_by, mean_value, fill = taxon)) +
  ...
  theme(axis.title.y = element_text(face = "bold", size = 12),
        ...
        panel.spacing.x = unit(c(0.2, 0.2, 0.2), "lines"))
phyplot2
pdf("InitialFigs/Phylum_Barplot_Other_10.pdf", width = 8, height = 6)
phyplot2
dev.off()
png("InitialFigs/Phylum_Barplot_Other_10.png", width = 8, height = 6, units = "in", res = 300)
phyplot2
dev.off()

pdf("InitialFigs/Figure9.pdf", width = 8.5, height = 9)
plot_grid(phyplot, phyplot2, labels = "AUTO")
dev.off()
png("InitialFigs/Figure9.png", width = 8.5, height = 6, units = "in", res = 300)
plot_grid(phyplot, phyplot2, labels = "AUTO")
dev.off()
...
plot.with.inset
pdf("InitialFigs/FigureS7.pdf", width = 7, height = 5)
plot.with.inset
dev.off()
png("InitialFigs/FigureS7.png", width = 7, height = 5, units = "in", res = 300)
plot.with.inset
dev.off()
...
plot.with.inset.30
pdf("InitialFigs/Beta_30.pdf", width = 7, height = 5)
plot.with.inset.30
dev.off()
pdf("InitialFigs/Beta_30_wVec.pdf", width = 7, height = 5)
plot.with.inset.30
dev.off()
png("InitialFigs/Beta_30_wVec.png", width = 7, height = 5, units = "in", res = 300)
plot.with.inset.30
dev.off()
...
pheatmap(simper_mat,
         ...
         display_numbers = T,
         gaps_row = c(10, 20),
         filename = "InitialFigs/Simper_Prok.png",
         width = 5,
         height = 5)
dev.off()
dev.set(dev.next())
...
out <- ancombc2(data = input.phy,
                ...
                n_cl = 1, 
                verbose = TRUE)
saveRDS(out, file = "data/ancom_out_prok_all.rds")
...
png("InitialFigs/VennDiagram.png", width = 7, height = 5, units = "in", res = 300)
ggvenn(venn_list,
...
pheatmap(ancom_mat,
         ...
         display_numbers = T,
         gaps_row = c(71, 157),
         filename = "InitialFigs/Ancom_Prok.png",
         width = 8,
         height = 12)
dev.off()
dev.set(dev.next())
...
pdf("InitialFigs/AncomLFC_Prok.pdf", width = 8, height = 12)
...
dev.off()
png("InitialFigs/AncomLFC_Prok.png", width = 8, height = 12, units = "in", res = 300)
ggplot(ancom_results, aes(reorder(Taxonomy, LFC), LFC)) +
  coord_flip() +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black", linewidth = 0.5) +
  geom_bar(stat = "identity") +
...
pdf("InitialFigs/AncomLFC_Prok_tax.pdf", width = 8, height = 12)
ggplot(ancom_results, aes(Taxonomy, LFC)) +
  ...
  theme(axis.text.y = element_text(size = 4),
        ...
        panel.grid.minor = element_blank())
dev.off()
png("InitialFigs/AncomLFC_Prok_tax.png", width = 8, height = 12, units = "in", res = 300)
ggplot(ancom_results, aes(Taxonomy, LFC)) +
  coord_flip() +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black", linewidth = 0.5) +
  geom_bar(stat = "identity") +
...
ancom_results2 <- ancom_results %>%
  ...
  mutate(Comb = paste(`Control_+Basalt`, `Control_+MP1`, `Control_+MP1+Basalt`, 
                      sep = "_"))
levels(as.factor(ancom_results2$Comb)) # 11 combinations, too many
tax_sum_otu <- summarize_taxonomy(top, 
                                  level = 8, 
                                  relative = T, 
                                  report_higher_tax = F)
...
pdf("InitialFigs/SigTaxa_Abund_Basalt.pdf", width = 7, height = 5)
ggplot(ancom_tax_sum, aes(Treatment, value*100)) +
...
ancom_tax_sum <- tax_sum_otu %>%
  ...
  pivot_longer(cols = c(1:13)) %>%
  left_join(., info, by = c("name" = "OTU"))
pdf("InitialFigs/SigTaxa_Abund_MP1.pdf", width = 7, height = 5)
ggplot(ancom_tax_sum, aes(Treatment, value*100)) +
  geom_boxplot(outlier.shape = NA, aes(colour = Treatment), show.legend = F) +
  geom_jitter(size = 1, pch = 16, alpha = 1, width = 0.2, height = 0, 
              aes(colour = Treatment), show.legend = F) +
...
ancom_tax_sum <- tax_sum_otu %>%
  ...
  pivot_longer(cols = c(1:13)) %>%
  left_join(., info, by = c("name" = "OTU"))
pdf("InitialFigs/SigTaxa_Abund_MP1Basalt.pdf", width = 7, height = 5)
ggplot(ancom_tax_sum, aes(Treatment, value*100)) +
  geom_boxplot(outlier.shape = NA, aes(colour = Treatment), show.legend = F) +
  geom_jitter(size = 1, pch = 16, alpha = 1, width = 0.2, height = 0, 
              aes(colour = Treatment), show.legend = F) +
...
pheatmap(tax_sum_otu_DA,
         ...
         na_col = "white",
         border_color = "white",
         filename = "InitialFigs/ANCOM_Heatmap_NoClust.png",
         width = 10,
         height = 8)
dev.off()
dev.set(dev.next())
...
#### __Euk ####
# Not many reads, didn't end up using.
...
bars_phyla <- bars_phyla %>%
  mutate(taxon = factor(taxon,
                        levels = c("Other", rev(top_phyla$taxon))))
pdf("InitialFigs/Taxa_Phyla_Euk.pdf", width = 8, height = 6)
ggplot(bars_phyla, aes(group_by, mean_value, fill = taxon)) +
  geom_bar(stat = "identity", colour = NA, size = 0.25) +
  labs(x = "Sample", y = "Relative abundance", fill = "Phylum") +
  scale_fill_manual(values = c("grey90", brewer.pal(12, "Paired")[12:1])) +
...
pheatmap(sylph_strains_70,
         ...
         na_col = "white",
         border_color = "white",
         filename = "InitialFigs/Sylph_Comb.png",
         width = 10,
         height = 13)
dev.off()
dev.set(dev.next())
...
pheatmap(sylph_strains_70_Bac,
         ...
         border_color = "white",
         show_colnames = F,
         filename = "InitialFigs/FigureS5.png",
         width = 10,
         height = 8)
dev.off()
dev.set(dev.next())
...
mp1.sylph <- ggplot(d_sort, aes(Treatment, MP1_sylph, shape = Depth)) +
  ...
  theme(legend.position = "inside",,
        ...
        panel.grid = element_blank())
mp1.sylph
pdf("InitialFigs/MP1_sylph_comb.pdf", width = 7, height = 5)
mp1.sylph
dev.off()
png("InitialFigs/MP1_sylph_comb.png", width = 7, height = 5, units = "in", res = 300)
mp1.sylph
dev.off()
